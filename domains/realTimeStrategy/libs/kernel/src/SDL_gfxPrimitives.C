// $Id: SDL_gfxPrimitives.C 5279 2007-06-23 02:49:08Z mburo $
...
int filledRTriangle (SDL_Surface *dst, Sint16 x, Sint16 y, Sint16 dx, Sint16 dy, int color)
{
  int result = 0;
  int len = abs(dx);
  ...
  for (int i = 0; i < len; ++i) {
    xlen += xx;
    result |= hlineColor(dst, x, x+xlen, y+i*yy, color);
  }
  return (result);
}
